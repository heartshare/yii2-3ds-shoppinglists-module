...
namespace yii3ds\shoppinglists\behaviors;
 ...
 * ```
 */
class UploadBehavior3ds extends UploadBehavior
{
    /**
    ...
    protected function saveFile($attribute, $insert = true)
    {
        if (empty($this->owner->$attribute)) {
            ...
            if (is_file($tempFile) && 
            	FileHelper::createDirectory($this->path($attribute))
            	) {
    ...
    {
    	$arr = explode('/', $this->owner->$attribute);
    	$fileName = end($arr);
